...
### 2. End Element
- Marks the absolute termination of the workflow process
- No lock_keyword or user_assigned required
- Terminal point in the flow — it is NOT a form state
- Must always be preceded by a `process` element (the final form state), never connected directly from a decision or system element

### 3. Process Element
- Represents a **form state** — the form exists in this state while the assigned user performs actions within it
- The form remains in this state until the user triggers a transition (button, time, or await) that moves it to another process element or to end
- Requires lock_keyword and user_assigned
- **What belongs in the main diagram:** any `system_decision` or `system_action` element that sits on the transition path between one `user_action_process` and the next is part of the main diagram — regardless of whether it is automated. If it is executed as part of moving the form from one state to another, it must appear as an element with connections.
- **What does NOT belong in the main diagram:** actions that happen entirely within a single form state and do not lead to any other element — these are internal subprocesses of that form state and must not appear as diagram elements or connections.
- A finalized or closed form is itself a process element (form state), not an end element. It has its own outbound connection to the end element.
...
- A closed or finalized form state must be modelled as a `process` element, not as an `end` element
...
- Only transitions that move the form to another process state or to end belong here — internal form actions that do not lead to any other element are not connections
...
| PROC_CLOSED | Closed Invoice | process | lock_noteditable | System | Archived closed invoice | |
...
| CONN_INV_003 | PROC_REVIEW | PROC_CLOSED | | normal | Approve Form | | button |
| CONN_INV_004 | PROC_CLOSED | END_INV | | normal | | | await |
